...
void mat_mul(int size, int rank) {
	...
	MPI_Ibcast(a, N*N, MPI_FLOAT, 0, MPI_COMM_WORLD, &request);
	MPI_Wait(&request, &status);
	MPI_Ibcast(b, N*N, MPI_FLOAT, 0, MPI_COMM_WORLD, &request);
	MPI_Wait(&request, &status);
	...
	MPI_Igather(&c[rank*N/size][0], N*N/size, MPI_FLOAT, c, N*N/size, MPI_FLOAT, 0, MPI_COMM_WORLD, &request);
	if(rank == 0)
	MPI_Wait(&request, &status);
